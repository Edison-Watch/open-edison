...
from src.mcp_importer.api import restore_client
...
async def detect_available_clients():
    ...
    try:
        ...
        return ClientDetectionResponse(
            ...
            message=f"Found {len(client_names)} MCP clients"
        )
    except Exception as e:
        return ClientDetectionResponse(
            success=False,
            clients=[],
            message=f"Error detecting clients: {str(e)}"
        )
    ...
    try:
        ...
        for client_name in request.clients:
            try:
                ...
                errors.append(f"Error importing from {client_name}: {str(e)}")
        
        # Deduplicate servers
        all_servers = deduplicate_by_name(all_servers)
        
        # Convert to API format
        server_configs: List[ServerConfig] = [convert_to_server_config(server) for server in all_servers]
...
async def verify_servers(request: VerificationRequest):
    ...
    try:
        ...
        success_count = sum(1 for valid in results.values() if valid is True)
...
async def authorize_oauth(request: OAuthRequest):
    ...
    try:
        ...
        return OAuthResponse(
            success=False,
            message=f"OAuth authorization error: {str(e)}"
        )
...
async def save_imported_servers_to_config(servers: List[ServerConfig], dry_run: bool = False):
    ...
    try:
        ...
        return {
            "success": False,
            "message": f"Failed to save servers: {str(e)}"
        }
...
async def export_to_clients(request: ExportRequest):
    ...
    try:
        ...
        success_count = sum(1 for result in results.values() if isinstance(result, dict) and result.get("success", False))
...
@app.post("/restore")
async def restore_client_configs(clients: List[str], server_name: str = "open-edison", dry_run: bool = False):
        ...
        for client_name in clients:
            try:
                ...
                results[client_name] = {
                    ...
                    "backup_path": str(getattr(result, 'backup_path', None)) if getattr(result, 'backup_path', None) else None,
