...
def test_import_and_save_cursor(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("src.mcp_importer.importers.find_cursor_user_file", lambda: [editor_file])
...
def test_export_to_cursor_overwrites_to_open_edison(
    ...
    monkeypatch.setattr("src.mcp_importer.exporters.find_cursor_user_file", lambda: [editor_file])
    monkeypatch.setattr("src.mcp_importer.exporters._resolve_cursor_target", lambda: editor_file)
...
def test_detect_clients_with_patched_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "src.mcp_importer.paths.detect_cursor_config_path",
        lambda: tmp_path / "fake-cursor.json",
    )
    monkeypatch.setattr(
        "src.mcp_importer.paths.detect_vscode_config_path",
        lambda: tmp_path / "fake-vscode.json",
    )
    monkeypatch.setattr(
        "src.mcp_importer.paths.detect_claude_code_config_path",
